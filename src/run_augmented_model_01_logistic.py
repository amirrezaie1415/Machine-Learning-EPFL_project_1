...
fileDir = os.path.dirname(os.path.abspath('run_augmented_model_01_logistic.py'))
...
# transforming data using gaussian basis
basis_list = [3, 5, 8, 10, 15, 20, 30]
...
# preallocate weights, training loss and validation loss, train acc. and valid acc.
w_0, w_1, w_2, w_3 = np.zeros((4, len(lambda_list), len(basis_list)), dtype='object')
tr_loss_0, tr_loss_1, tr_loss_2, tr_loss_3 = np.zeros((4, len(lambda_list), len(basis_list)))
val_loss_0, val_loss_1, val_loss_2, val_loss_3 = np.zeros((4, len(lambda_list), len(basis_list)))
accuracy_train_0, accuracy_train_1, accuracy_train_2, accuracy_train_3 = np.zeros((4, len(lambda_list), len(basis_list)))
accuracy_valid_0, accuracy_valid_1, accuracy_valid_2, accuracy_valid_3 = np.zeros((4, len(lambda_list), len(basis_list)))

# looping over the different number of gaussian basis function
for counter_basis, basis_number in enumerate(basis_list):
    print('[INFO]: Start learning models with augmented features using gaussian basis functions "{}"'.format(
        basis_number))
    ...
    X_tr_aug_0, X_val_aug_0 = build_gaussian_basis(X_tr_stan_0, basis_number), build_gaussian_basis(X_val_stan_0,
                                                                                                   basis_number)
    X_tr_aug_1, X_val_aug_1 = build_gaussian_basis(X_tr_stan_1, basis_number), build_gaussian_basis(X_val_stan_1,
                                                                                                   basis_number)
    X_tr_aug_2, X_val_aug_2 = build_gaussian_basis(X_tr_stan_2, basis_number), build_gaussian_basis(X_val_stan_2,
                                                                                                    basis_number)
    X_tr_aug_3, X_val_aug_3 = build_gaussian_basis(X_tr_stan_3, basis_number), build_gaussian_basis(X_val_stan_3,
                                                                                                   basis_number)
    ...
    for counter_lambda, lambda_ in enumerate(lambda_list):
        ...
        w_0[counter_lambda, counter_basis], tr_loss_0[counter_lambda, counter_basis] = reg_logistic_regression(y_tr_0, X_tr_aug_0, lambda_, initial_w, max_iters, gamma)
        val_loss_0[counter_lambda, counter_basis] = compute_loss_logistic(y_val_0, X_val_aug_0, w_0[counter_lambda, counter_basis])
        y_pred_train_0 = predict_logistic(X_tr_aug_0, w_0[counter_lambda, counter_basis])
        # map to {0, 1}
        y_pred_train_0[y_pred_train_0 > 0] = 1
        y_pred_train_0[y_pred_train_0 < 0] = 0

        y_pred_val_0 = predict_logistic(X_val_aug_0, w_0[counter_lambda, counter_basis])
        # map to {0, 1}
        y_pred_val_0[y_pred_val_0 > 0] = 1
        y_pred_val_0[y_pred_val_0 < 0] = 0
        accuracy_train_0[counter_lambda, counter_basis] = np.where(y_tr_0 - y_pred_train_0 == 0)[0].shape[0]/y_tr_0.shape[0]
        accuracy_valid_0[counter_lambda, counter_basis] = np.where(y_val_0 - y_pred_val_0 == 0)[0].shape[0]/y_val_0.shape[0]
        print('[INFO]: Accuracy_train = {:.3f}, Accuracy_val = {:.3f}'.format(accuracy_train_0[counter_lambda, counter_basis],accuracy_valid_0[counter_lambda,counter_basis]))
        ...
        w_1[counter_lambda, counter_basis], tr_loss_1[counter_lambda, counter_basis] = reg_logistic_regression(y_tr_1, X_tr_aug_1, lambda_, initial_w, max_iters, gamma)
        val_loss_1[counter_lambda, counter_basis] = compute_loss_logistic(y_val_1, X_val_aug_1, w_1[counter_lambda, counter_basis])
        y_pred_train_1 = predict_logistic(X_tr_aug_1, w_1[counter_lambda, counter_basis])
        # map to {0, 1}
        y_pred_train_1[y_pred_train_1 > 0] = 1
        y_pred_train_1[y_pred_train_1 < 0] = 0

        y_pred_val_1 = predict_logistic(X_val_aug_1, w_1[counter_lambda, counter_basis])
        # map to {0, 1}
        y_pred_val_1[y_pred_val_1 > 0] = 1
        y_pred_val_1[y_pred_val_1 < 0] = 0
        accuracy_train_1[counter_lambda, counter_basis] = np.where(y_tr_1 - y_pred_train_1 == 0)[0].shape[0]/y_tr_1.shape[0]
        accuracy_valid_1[counter_lambda, counter_basis] = np.where(y_val_1 - y_pred_val_1 == 0)[0].shape[0]/y_val_1.shape[0]
        print('[INFO]: Accuracy_train = {:.3f}, Accuracy_val = {:.3f}'.format(accuracy_train_1[counter_lambda, counter_basis],accuracy_valid_1[counter_lambda, counter_basis]))
        ...
        w_2[counter_lambda, counter_basis], tr_loss_2[counter_lambda, counter_basis] = reg_logistic_regression(y_tr_2, X_tr_aug_2, lambda_, initial_w, max_iters, gamma)
        val_loss_2[counter_lambda, counter_basis] = compute_loss_logistic(y_val_2, X_val_aug_2, w_2[counter_lambda, counter_basis])
        y_pred_train_2 = predict_logistic(X_tr_aug_2, w_2[counter_lambda, counter_basis])
        # map to {0, 1}
        y_pred_train_2[y_pred_train_2 > 0] = 1
        y_pred_train_2[y_pred_train_2 < 0] = 0

        y_pred_val_2 = predict_logistic(X_val_aug_2, w_2[counter_lambda, counter_basis])
        # map to {0, 1}
        y_pred_val_2[y_pred_val_2 > 0] = 1
        y_pred_val_2[y_pred_val_2 < 0] = 0
        accuracy_train_2[counter_lambda, counter_basis] = np.where(y_tr_2 - y_pred_train_2 == 0)[0].shape[0]/y_tr_2.shape[0]
        accuracy_valid_2[counter_lambda, counter_basis] = np.where(y_val_2 - y_pred_val_2 == 0)[0].shape[0]/y_val_2.shape[0]
        print('[INFO]: Accuracy_train = {:.3f}, Accuracy_val = {:.3f}'.format(accuracy_train_2[counter_lambda, counter_basis], accuracy_valid_2[counter_lambda, counter_basis]))
        ...
        w_3[counter_lambda, counter_basis], tr_loss_3[counter_lambda, counter_basis] = reg_logistic_regression(y_tr_3, X_tr_aug_3, lambda_, initial_w, max_iters, gamma)
        val_loss_3[counter_lambda, counter_basis] = compute_loss_logistic(y_val_3, X_val_aug_3, w_3[counter_lambda, counter_basis])
        y_pred_train_3 = predict_logistic(X_tr_aug_3, w_3[counter_lambda, counter_basis])
        # map to {0, 1}
        y_pred_train_3[y_pred_train_3 > 0] = 1
        y_pred_train_3[y_pred_train_3 < 0] = 0

        y_pred_val_3 = predict_logistic(X_val_aug_3, w_3[counter_lambda, counter_basis])
        # map to {0, 1}
        y_pred_val_3[y_pred_val_3 > 0] = 1
        y_pred_val_3[y_pred_val_3 < 0] = 0
        accuracy_train_3[counter_lambda, counter_basis] = np.where(y_tr_3 - y_pred_train_3 == 0)[0].shape[0]/y_tr_3.shape[0]
        accuracy_valid_3[counter_lambda, counter_basis] = np.where(y_val_3 - y_pred_val_3 == 0)[0].shape[0]/y_val_3.shape[0]
        print('[INFO]: Accuracy_train = {:.3f}, Accuracy_val = {:.3f}'.format(accuracy_train_3[counter_lambda, counter_basis], accuracy_valid_3[counter_lambda, counter_basis]))
...
f.write('##################################################\n\n')
f.write('***Augmented model 01***\n\n')
for counter_basis, basis_number in enumerate(basis_list):
    f.write('%Number of basis functions:' + str(basis_number) + '\n\n')
    for counter_lambda, lambda_ in enumerate(lambda_list):
        f.write('\t%lambda:' + str(lambda_) + '\n')
        f.write('\t\t-Train Accuracy:    #Group0:' + '{:.3f}'.format(accuracy_train_0[counter_lambda, counter_basis]) + '    #Group1:' + '{:.3f}'.format(accuracy_train_1[counter_lambda, counter_basis]) + '    #Group2:' + '{:.3f}'.format(accuracy_train_2[counter_lambda, counter_basis]) + '    #Group3:' + '{:.3f}'.format(accuracy_train_3[counter_lambda, counter_basis]) + '\n')
        f.write('\t\t-Valid Accuracy:    #Group0:' + '{:.3f}'.format(accuracy_valid_0[counter_lambda, counter_basis]) + '    #Group1:' + '{:.3f}'.format(accuracy_valid_1[counter_lambda, counter_basis]) + '    #Group2:' + '{:.3f}'.format(accuracy_valid_2[counter_lambda, counter_basis]) + '    #Group3:' + '{:.3f}'.format(accuracy_valid_3[counter_lambda, counter_basis]) + '\n\n')
    f.write('---------------------------------------------------------------------------------------------\n\n')
f.close()
...
# find the best number of basis function for each group
...
best_basis_2 = basis_list[np.where(val_loss_2 == np.min(val_loss_2))[1][0]]
best_basis_3 = basis_list[np.where(val_loss_3 == np.min(val_loss_3))[1][0]]

# transforming test data
X_test_aug_0 = build_gaussian_basis(X_test_0, best_basis_0)
X_test_aug_1 = build_gaussian_basis(X_test_1, best_basis_1)
X_test_aug_2 = build_gaussian_basis(X_test_2, best_basis_2)
X_test_aug_3 = build_gaussian_basis(X_test_3, best_basis_3)

    
# adding the intercept term
X_test_aug_0 = np.c_[np.ones(X_test_aug_0.shape[0]), X_test_aug_0]
X_test_aug_1 = np.c_[np.ones(X_test_aug_1.shape[0]), X_test_aug_1]
X_test_aug_2 = np.c_[np.ones(X_test_aug_2.shape[0]), X_test_aug_2]
X_test_aug_3 = np.c_[np.ones(X_test_aug_3.shape[0]), X_test_aug_3]


# predicting test data
y_test_pred_0 = predict_logistic(X_test_aug_0, w_0[np.where(val_loss_0 == np.min(val_loss_0))[0][0], np.where(val_loss_0 == np.min(val_loss_0))[1][0]])
y_test_pred_1 = predict_logistic(X_test_aug_1, w_1[np.where(val_loss_1 == np.min(val_loss_1))[0][0], np.where(val_loss_1 == np.min(val_loss_1))[1][0]])
y_test_pred_2 = predict_logistic(X_test_aug_2, w_2[np.where(val_loss_2 == np.min(val_loss_2))[0][0], np.where(val_loss_2 == np.min(val_loss_2))[1][0]])
y_test_pred_3 = predict_logistic(X_test_aug_3, w_3[np.where(val_loss_3 == np.min(val_loss_3))[0][0], np.where(val_loss_3 == np.min(val_loss_3))[1][0]])
...
OUTPUT_PATH = os.path.join(parentDir, 'data/submission_augmented_model_01_logistic.csv')
